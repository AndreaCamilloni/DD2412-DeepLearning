...
def main_worker(gpu, args, ngpus_per_node=None):
    ...
    if not args.no_freeze:
        ...
        for name, param in model.named_parameters():
            if args.arch == 'alexnet':
                fc_weights_bias = ['classifier.6.weight','classifier.6.bias']
            else:
                fc_weights_bias = ['fc.weight','fc.bias']
                
            if name not in fc_weights_bias:
    ...
    if args.pretrained is not None:
        if os.path.isfile(args.pretrained):
            ...
            if args.arch == 'alexnet':
                assert set(msg.missing_keys) == {'classifier.6.weight','classifier.6.bias'} 
            else: 
...
def sanity_check(state_dict, pretrained_weights,args):
    ...
    for k in list(state_dict.keys()):
        ...
        if args.arch == 'alexnet':
            if 'classifier.6.weight' in k or 'classifier.6.bias' in k:
                continue
