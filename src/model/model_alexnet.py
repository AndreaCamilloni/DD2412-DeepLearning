...
class Model(nn.Module):
    ...
    def forward(self, x, return_embds=False):
        if isinstance(x, list):  # multiple views
            bs_size = x[0].shape[0] #bs is batch size
            if return_embds:
                ...
                for end_idx in idx_crops:
                    ...
                    #print("#"*10,"Check Point 2",_out.shape)
                    if start_idx == 0:
                        output = _out
                    else:
                ...
                #print("#"*10,"Check Point 3")
                embds = self.classifier_head(output)
                #print("#"*10,"Check Point 4")
...
class MLPhead(nn.Module):
    def __init__(self, input_size, output_size, num_hidden_layers=2, hidden_size=256, activation='relu', batch_norm=False):
        ...
        #print("#"*10,"Check Point 5")
        if num_hidden_layers == 0:
            #print("#"*10,"Check Point 11")
            self.mlp = nn.Identity()
        elif num_hidden_layers == 1:
            #print("#"*10,"Check Point 6")
            self.mlp = nn.Linear(input_size, output_size)
        else:
            #print("#"*10,"Check Point 10")
            #print('input size',input_size,'hidden_size',hidden_size)
...
#                nn.init.zeros_(m.bias)
    ...
    def forward(self, x):
        #print("#"*10,"Check Point 8")
        x = self.mlp(x)
        #print("#"*10,"Check Point 9")
